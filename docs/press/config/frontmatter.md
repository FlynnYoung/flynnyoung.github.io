...
# 文章与主题配置查询
:::tip  作者声明
该篇文章转载至[@sugarat/theme](https://theme.sugarat.top/)，目的是为了方便作者在写文章时，方便查询配置项,不用每次都打开原文查询。原文请查看[@sugarat/theme/主题配置](https://theme.sugarat.top/config/frontmatter.html).
:::
...
### hiddenCover
`hiddenCover` 控制是否展示当前文章的封面，全局配置开关见 [article.hiddenCover](https://theme.sugarat.top/config/global.html#article)
```md
---
hiddenCover: true 
cover: url
...
如果没有单独设置则会默认使用全局的 `author` 配置，这个在 [全局配置: author](https://theme.sugarat.top/config/global.html#author) 有介绍
